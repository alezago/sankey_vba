...
Private Const HIGH_TRANSPARENCY As Double = 0.95
Private Const LOW_TRANSPARENCY As Double = 0.05

'Color Settings Defaults
'all values are [0,255] unless otherwise specified
'colors are mainly handled in HSL mode, RGB conversion is done internally
Private Const SATURATION_MEDIAN As Integer = 200    'median saturation for all the levels [0-255]
Private Const SATURATION_VARIANCE As Integer = 10    'max. possible variance in saturation between blocks [0-255]
Private Const LUMINANCE_MEDIAN As Integer = 150    'median luminance for all the levels [0-255]
Private Const LUMINANCE_VARIANCE As Integer = 10    'max. possible variance in luminance between blocks
Private Const HUE_VARIANCE_IN_LEVEL As Integer = 25    'variance in hue within one level of blocks. Block colors over one level will be spread equally over this range. [0-255]
Private Const HUE_VARIANCE_BETWEEN_LEVELS As Integer = 50    'jump in hue between the base color of one level of blocks and the base color of the next one. [0-255]
...
Private Const BG_COLOR_R As Integer = 45
Private Const BG_COLOR_G As Integer = 45
Private Const BG_COLOR_B As Integer = 45
...
Private Function drawSankey(data As Range, targetSheet As Worksheet, chartName As String, chartTop As Double, chartLeft As Double, chartWidth As Double, chartHeight As Double, skc As sankeyConfig)
    ...
    setPropertyValue shapeGroup, "TR_BASE", CStr(Round(skc.baseTransparency, 2))
    setPropertyValue shapeGroup, "TR_LOW", CStr(Round(skc.lowTransparency, 2))
    setPropertyValue shapeGroup, "TR_HIGH", CStr(Round(skc.highTransparency, 2))
...
'this function is the public interface to the drawSankey function
...
drawSankey data, targetSheet, chartName, chartTop, chartLeft, chartWidth, chartHeight, skc

End Function

'returns a sankey config. structure with default values
Public Function getDefaultSankeyConfig() As sankeyConfig

'Background Color (dark grey)
Dim bgColor As colorRGB
bgColor.r = BG_COLOR_R
bgColor.g = BG_COLOR_G
bgColor.b = BG_COLOR_B

getDefaultSankeyConfig.verticalBlankSpacePerc = VERTICAL_SPACING_PERC    '% of blank space between blocks, compared to the total available space
getDefaultSankeyConfig.hMargin = HORIZONTAL_MARGIN
getDefaultSankeyConfig.vMargin = VERTICAL_MARGIN
getDefaultSankeyConfig.blockColorMode = gradientColor
getDefaultSankeyConfig.lowTransparency = LOW_TRANSPARENCY
getDefaultSankeyConfig.baseTransparency = BASE_TRANSPARENCY
getDefaultSankeyConfig.highTransparency = HIGH_TRANSPARENCY

    'color settings
getDefaultSankeyConfig.saturationMedian = SATURATION_MEDIAN
getDefaultSankeyConfig.saturationVariance = SATURATION_VARIANCE
getDefaultSankeyConfig.luminanceMedian = LUMINANCE_MEDIAN
getDefaultSankeyConfig.luminanceVariance = LUMINANCE_VARIANCE
getDefaultSankeyConfig.hueRangeForLevel = HUE_VARIANCE_IN_LEVEL
getDefaultSankeyConfig.hueVarianceBetweenLevels = HUE_VARIANCE_BETWEEN_LEVELS
getDefaultSankeyConfig.backgroundColor = bgColor
...
Private Function handleBlockClick(block As Shape)
    ...
    Dim lowTransparency As Double, baseTransparency As Double, highTransparency As Double

    chartID = getPropertyValue(block, "CHARTID")
    element = getPropertyValue(block, "ELEMENT", "")
    level = getPropertyValue(block, "LEVEL", "0")

    'get the color settings from the background shape
    Set backgroundShape = ActiveSheet.Shapes(SHAPE_NAME_PREFIX & chartID & "_BACKGROUND")

    lowTransparency = getPropertyValue(backgroundShape, "TR_LOW", "1")
    baseTransparency = getPropertyValue(backgroundShape, "TR_BASE", "1")
    highTransparency = getPropertyValue(backgroundShape, "TR_HIGH", "1")



    block.Fill.Transparency = lowTransparency
    block.Line.Transparency = highTransparency
    ...
    For Each sh In groupShp.GroupItems
        ...
        If blockType = "BLOCK" Then
         sh.Fill.Transparency = highTransparency
         sh.Line.Transparency = highTransparency
    ...
    For Each sh In groupShp.GroupItems
        If getPropertyValue(sh, "TYPE") = "CONNECTOR" Then
            ...
            If getPropertyValue(sh, "LEVEL" & level) = element Then
                sh.Fill.Transparency = lowTransparency
                sh.Line.Transparency = lowTransparency
                ...
                sourceBlock.Fill.Transparency = lowTransparency
                sourceBlock.Line.Transparency = lowTransparency

                targetBlock.Fill.Transparency = lowTransparency
                targetBlock.Line.Transparency = lowTransparency
